...
const CreatePOLinesModal = ({
 ...
 const [lines, setLines] = useState([
    {
      po_lnum: generatePOLineNumber(),
      header_id: headerId,
      item_id: "",
      quantity: "",
      uom_id: "", // Include uom_id
      unit_price: "",
      tax_id: taxId,
      notes: "",
      line_total: 0,
  ...
  const handleCreateLines = async () => {
    ...
    try {
      const response = await axios.post(
        `${API_URL}/create_purchase_order_line`,
        {
          purchase_order_lines: lines.map((line) => ({
            ...line,
            header_id: headerId,
            status: hdr_status,
          })),
      ...
      if (response.data.success) {
        ...
        console.error(
          "Error creating purchase order lines:",
          response.data.message
        );
      }
    } catch (error) {
      console.error("Error creating purchase order lines:", error);
    }
  };
  ...
  return (
    ...
    >
      <Modal.Header closeButton className="po-line-modal-title-custom">
        <Modal.Title>
          Create Purchase Order Lines for PO Number ({poNumber})
        </Modal.Title>
      </Modal.Header>
      <Modal.Body>
...
export default CreatePOLinesModal;
