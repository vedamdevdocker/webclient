...
import {
  UPDATABLE_PURCHASE_INVOICE_STATUS,
} from "../../config/config";
import logger from "../../../utilities/Logs/logger";
import UpdatePurchaseLineModalForm from "./UpdatePurchaseLineModalForm";
...
export default function UpdatePOInvoiceHeaderForm() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        apiUrl += UPDATABLE_PURCHASE_INVOICE_STATUS.map(status => `status=${status.code}`).join('&');
        ...
        if (response.data.purchase_invoice_headers.length > 0) {
          const firstPurchaseInvoice =
            response.data.purchase_invoice_headers[0];
          const companyId = firstPurchaseInvoice.company_id;
          setInvoiceHeader(firstPurchaseInvoice.header_id);
          setSelectedCurrencyId(firstPurchaseInvoice.currency_id);
          setInvoiceNumber(firstPurchaseInvoice.invoice_number);
          setSelectedTaxId(firstPurchaseInvoice.tax_id);
          setTotalSum(firstPurchaseInvoice.totalamount);
          setInvoiceTotal(firstPurchaseInvoice.totalamount);
          setSelectedCompany(companyId)
          setSelectedDepartment(firstPurchaseInvoice.department_id)
          ...
          setFormData({
            company_id: firstPurchaseInvoice.company_id,
            department_id: firstPurchaseInvoice.department_id,
            currency_id: firstPurchaseInvoice.currency_id,
            status: firstPurchaseInvoice.status,
            tax_id: firstPurchaseInvoice.tax_id,
            totalamount: firstPurchaseInvoice.totalamount,
            partnerid: firstPurchaseInvoice.partnerid,
            invoicedate: new Date(firstPurchaseInvoice.invoicedate)
              .toISOString()
              .split("T")[0],
            payment_duedate: new Date(firstPurchaseInvoice.payment_duedate)
              .toISOString()
              .split("T")[0],
            payment_terms: firstPurchaseInvoice.payment_terms,
          });
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (formData.status === "APPROVED") {
      try {
        ...
        validateResponse = await axios.get(
          `${API_URL}/validate_po_invoice?header_id=${invoiceHeader}`,
          { headers: generateHeaders() }
        );
        ...
        console.error("Error validating Purchase Invoice:", error.response.data.error);
        if (error.response && error.response.data && error.response.data.error) {
          ...
          setError("An error occurred while validating the purchase invoice.");
        }
        return;
      }
    }
    ...
    try {
      let apiUrl = `${API_URL}/update_purchase_invoice`;
      ...
      if (response.data.status) {
        ...
        setError(response.data.error || "Unknown error occurred while updating the purchase invoice");
      }
      setIsFormSubmitted(true);
      ...
      setError("An error occurred while updating the purchase invoice.", error);
    }
  };
            ...
            <div className="form-group col-md-6 mb-2">
              <div className="form-row">
                ...
                >
                  ...
                  {UPDATABLE_PURCHASE_INVOICE_STATUS.filter(
