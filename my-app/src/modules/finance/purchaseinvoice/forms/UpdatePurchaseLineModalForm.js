...
const UpdatePurchaseLineModalForm = ({
  ...
  useEffect(() => {
    const fetchInvoiceLines = async () => {
      try {
        ...
        setLines(response.data.purchase_invoice_lines);
        console.log(response.data.purchase_invoice_lines);
        calculateTotalLineAmount(response.data.purchase_invoice_lines);
      } catch (error) {
    ...
    try {
      ...
      if (updatedLines.length > 0) {
        ...
        const updateResponse = await axios.put(
          `${API_URL}/update_purchase_invoice_lines`,
          requestBody,
          { headers: generateHeaders() }
        );
    ...
    if (lineToDelete.line_id) {
      ...
      try {
        const response = await axios.delete(
          `${API_URL}/delete_purchase_invoice_lines`,
          {
            headers: generateHeaders(),
            data: {
...
export default UpdatePurchaseLineModalForm;
