...
function CreateBOMForm() {
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Prepare a map to track ParentItems and their Levels
      const levelMap = { [modelItem]: 1 }; // Start with ModelItem at Level 1
  
      // Prepare subsequent rows starting from index 1
      const subsequentRows = bomRows.slice(1).map((row) => {
        const effectiveDate = row.EffectiveDate || calculateEffectiveDate();
        const endDate = row.EndDate || calculateEndDate(effectiveDate);
  
        // Check if the ParentItem already has a Level assigned
        const currentLevel = levelMap[row.ParentItem];
  
        // If Level does not exist for this ParentItem, assign next level
        if (currentLevel === undefined) {
          levelMap[row.ParentItem] = Object.keys(levelMap).length + 1; // Assign new Level
        }
        ...
        return {
          ...
          EffectiveDate: effectiveDate,
          EndDate: endDate,
          Level: levelMap[row.ParentItem].toString(), // Use the assigned Level
        };
      });
  
      // Prepare the final payload
      const payload = {
        bom: [firstRow, ...subsequentRows], // Include the first row followed by subsequent rows
      };
